...
<div class="kt-subheader__main">
	...
	<div class="kt-subheader__breadcrumbs">
		...
		<a href="" class="kt-subheader__breadcrumbs-link">Hostels</a>
			...
			<div class="kt-portlet__head kt-portlet__head--lg">
				...
				<div class="kt-portlet__head-toolbar">
					<div class="kt-portlet__head-wrapper">
						<a href="{{ route('hostels.add') }}" class="btn btn-brand btn-icon-sm">
							...
							<div class="row align-items-center">
								<div class="col-md-4 kt-margin-b-20-tablet-and-mobile">
									<div class="kt-input-icon kt-input-icon--left">
										<form action="{{ route('team.search') }}" method="POST" role="search">
										@csrf
			...
			<div class="kt-portlet__body">
				<div class="kt-section">
										...
										$id = '';
										 ...
										 <?php $id = $data->id; ?> 
												...
												<tr>
													<th scope="row">{{ $no }}</th>
													<td>{{ $data->name }}</td>
													<td>{{ $data->hostel_for }}</td>
										
													<td style=""><i class="fa {{ ($data->status=='active') ? 'fa-check text-success':'fa-times text-danger' }} inline"></i></td>


													<td>
					<a href="{{ route('hostels.view', ['id'=>$data->id]) }}" class="btn btn-outline-info btn-elevate btn-icon"><i class="flaticon-eye"></i></a>

					<a href="{{ route('hostels.edit', ['id'=>$data->id]) }}" class="btn btn-outline-warning btn-elevate btn-icon"><i class="flaticon-edit"></i></a>    
					...
					<button class="btn btn-outline-danger btn-elevate btn-icon" data-slug="{{ $data->id }}" data-toggle="modal"  data-target="#deletemodal"><i class="flaticon-delete"></i></button>
									...
									{{ $our_teams->appends(['sort' => 'votes'])->links() }}
								</div>
							</div>
						</div>
					</div>
...
<div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
		...
		<form action="{{ route('hostels.delete') }}" method="post">
        {{ method_field('delete')}}
		@csrf
